...
inline int RunNetConsumerProbe() noexcept
{
	...
	if (Loopback.TrySend(TSpan<const std::uint8_t>(FirstPacket.data(), FirstPacket.size())) != ENetResult::Success
		|| Loopback.TrySend(TSpan<const std::uint8_t>(SecondPacket.data(), SecondPacket.size())) != ENetResult::Success)
	{
	...
	if (Loopback.TryReceive(TSpan<std::uint8_t>(LoopbackDestination.data(), LoopbackDestination.size()), FirstReceive) != ENetResult::Success
		|| FirstReceive.BytesReceived != 2 || LoopbackDestination[0] != 0xAA)
	{
		return static_cast<int>(ENetConsumerExitCode::LoopbackFifoOrderBroken);
	}
	...
	if (Loopback.TryReceive(TSpan<std::uint8_t>(SecondDestination.data(), SecondDestination.size()), SecondReceive) != ENetResult::Success
		|| SecondReceive.BytesReceived != 2 || SecondDestination[0] != 0xCC)
	{
		return static_cast<int>(ENetConsumerExitCode::LoopbackFifoOrderBroken);
	}

	FNetReceiveResult EmptyReceive{};
	if (Loopback.TryReceive(TSpan<std::uint8_t>(LoopbackDestination.data(), LoopbackDestination.size()), EmptyReceive) != ENetResult::Unavailable)
	{
		return static_cast<int>(ENetConsumerExitCode::LoopbackEmptyDidNotReturnUnavailable);
	}

	// Full backpressure: a one-slot loopback must reject the second send and retain the head.
	FHostLoopback<1, 4> SingleLoopback;
	if (SingleLoopback.TrySend(TSpan<const std::uint8_t>(FirstPacket.data(), FirstPacket.size())) != ENetResult::Success
		|| SingleLoopback.TrySend(TSpan<const std::uint8_t>(SecondPacket.data(), SecondPacket.size())) != ENetResult::Full)
	{
	...
	if (SingleLoopback.TryReceive(TSpan<std::uint8_t>(TooSmall.data(), TooSmall.size()), TooSmallReceive) != ENetResult::Full)
	{
	...
	FNetManager<2, 4> Manager(Loopback, ManagerStorage);
	const std::array<std::uint8_t, 3> QueuePacket{0x01, 0x02, 0x03};
	if (Manager.QueueSend(TSpan<const std::uint8_t>(QueuePacket.data(), QueuePacket.size())) != ENetResult::Success)
	{
	...
	FHostLoopback<1, 4> BackpressureDriver;
	MicroWorld::FNetPacketStorage<1, 4> BackpressureStorage;
	FNetManager<1, 4> BackpressureManager(BackpressureDriver, BackpressureStorage);
	const std::array<std::uint8_t, 2> BackpressurePacket{0x55, 0x66};
	BackpressureDriver.TrySend(TSpan<const std::uint8_t>(BackpressurePacket.data(), BackpressurePacket.size()));
	BackpressureManager.QueueSend(TSpan<const std::uint8_t>(BackpressurePacket.data(), BackpressurePacket.size()));
	...
	if (Manager.Receive(TSpan<std::uint8_t>(ReceiveDestination.data(), ReceiveDestination.size()), ReceiveResult) != ENetResult::Success
