...
Phase 5 platform adapters are the next milestone: a `microworld-platform-host`
package (a `steady_clock` time source and a WinSock/BSD-socket UDP `INetDriver`)
first, then the ESP32 adapters (time, UDP, and the E32 LoRa UART driver), and a
small deterministic ESP32-S3 demonstration of the completed runtime. Platform
work may compile without upload unless hardware execution is explicitly
authorized.
