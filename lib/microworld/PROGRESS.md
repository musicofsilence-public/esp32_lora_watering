...
implementation candidates. The composed runtime's margins were first measured
on physical ESP32-S3 hardware in Phase 6.2 (tick / GC-slice / net-pump / heap /
stack — see the ESP32-S3 results file); isolated per-package margins beyond that
representative world remain unmeasured.

| Package | State |
| --- | --- |
| Core | Released 0.1 lifecycle and tick package |
| Memory | Implemented candidate; isolated runtime margins unmeasured |
| Object / GC | Implemented candidate; GC Advance-slice pause measured on ESP32-S3 (Phase 6.2): mean 25 µs/slice |
| Engine (incl. bounded timers) | Accepted implementation candidate; tick measured on ESP32-S3 (Phase 6.2): mean 73 µs (8 actors/16 components/8 timers) |
| Net | Accepted implementation candidate; no-traffic pump overhead measured on ESP32-S3 (Phase 6.2): mean 47 µs |
...
Phase 6 examples, measurement & release hardening is the next milestone: a
two-node demo (dedicated server on host UDP, ESP32 or second host as client),
measured ESP32-S3 runtime margins (tick duration, GC pause, net pump cost,
RAM/flash — requires explicit human authorization to flash), a documentation
release sweep, and final acceptance. Phase 5 platform adapters (host UDP,
ESP32 UDP, and the E32 LoRa UART transport with its portable, host-tested
`FrameCodec`) are complete and compile-only-verified; no UART traffic, radio,
or firmware upload has been exercised. Hardware execution requires explicit
authorization.
